...
export const POS: React.FC<POSProps> = ({ store, products, categories, invoices, onSaveInvoice, onUpdateProductStock }) => {
  ...
  const addToCart = (product: Product) => {
    if (product.stockQty <= 0) return; // Prevent adding if OOS
  ...
  return (
    <div className="h-[calc(100vh-64px)] flex flex-col -m-4 md:-m-8">
      ...
      <div className="bg-white border-b border-slate-200 px-8 py-3 flex items-center gap-4 shadow-sm z-10">
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                activeView === 'REGISTER' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'
            }`}
          >
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                activeView === 'HISTORY' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'
            }`}
          >
      ...
      <Modal isOpen={showPaymentModal} onClose={() => setShowPaymentModal(false)} title="Select Payment Method">
        ...
        <Button className="w-full py-3" onClick={finalizePayment} variant="success">
          Complete Payment
        </Button>
      </Modal>
